...
func _physics_process(delta):
	BleedCooldowns(delta);
	aiState._physics_process(delta);

func BleedCooldowns(delta):
	if(DifferenceRecalculationCooldown > 0):
		DifferenceRecalculationCooldown -= delta
	if(SeesPlayerCooldown > 0):
		SeesPlayerCooldown -= delta

func IsSeesPlayer(Player):
	if(SeesPlayerCooldown <= 0):
		...
		SeesPlayerCooldown = SeesPlayerTimeout
		SeesPlayer = intersected.empty()
	
	return SeesPlayer;
	
func IsNearPlayer(_Player):
	return NearPlayer
...
func TrackPlayer(Player):	
	if(DifferenceRecalculationCooldown <= 0):
		...
		DifferenceRecalculationCooldown = DifferenceRecalculationTimeout
