...
func _on_Ship_shoot(_BulletType, direction, location, velocity):
	var bullet = self.Bullet.instance()
...
func _on_LevelEndTrigger_body_shape_entered(body_id, body, body_shape, area_shape):
